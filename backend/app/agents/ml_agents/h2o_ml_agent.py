...
from app.tools.h2o import H2O_AUTOML_DOCUMENTATION
...
class H2OMLAgent(BaseAgent):
    ...
    def get_h2o_train_function(self, markdown=False):
        """Retrieves the H2O AutoML function code generated by the agent."""
        # First check self.response (for active sessions)
        ...
        # If not in self.response, check if we have session data
        # This handles cases where the agent is reconstructed from session
        if hasattr(self, '_session_data') and self._session_data:
            code = self._session_data.get("h2o_train_function")
            if code:
                if markdown:
                    return Markdown(f"```python\n{code}\n```")
                return code
...
def make_h2o_ml_agent(
    ...
    def execute_h2o_code(state):
        ...
        # CRITICAL FIX: Preserve the h2o_train_function from the state
        # This ensures the training function is available in the final response
        if "h2o_train_function" in state:
            result["h2o_train_function"] = state["h2o_train_function"]
        if "h2o_train_function_path" in state:
            result["h2o_train_function_path"] = state["h2o_train_function_path"]
        if "h2o_train_file_name" in state:
            result["h2o_train_file_name"] = state["h2o_train_file_name"]
        if "h2o_train_function_name" in state:
            result["h2o_train_function_name"] = state["h2o_train_function_name"]
